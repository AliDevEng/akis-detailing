import { WHATSAPP_MESSAGE, WHATSAPP_NUMBER } from "../constants/contact";
...
function FloatingWhatsApp() {
  ...
  return (
    <a
      ...
      className="group fixed bottom-[calc(env(safe-area-inset-bottom)+1rem)] right-4 z-40 flex items-center gap-2 rounded-full bg-[#25D366] py-3 pl-3 pr-4 text-white shadow-xl shadow-[#25D366]/30 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#25D366]/40 sm:bottom-6 sm:right-6"
    >
      <span className="absolute inset-0 -z-10 animate-ping rounded-full bg-[#25D366] opacity-30" />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 32 32"
        className="h-6 w-6 fill-white transition-transform duration-500 group-hover:rotate-[14deg] sm:h-7 sm:w-7"
        aria-hidden="true"
      >
